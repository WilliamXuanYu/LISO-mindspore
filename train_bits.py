...
if __name__ == "__main__":
    ...
    print(f"Train: {len(train.data_loader)} batches, Validation: {len(validation.data_loader)} images.")

    save_dir = get_path(args)
    print(save_dir)

    if args.eval and args.load is None:
        args.load = os.path.join(save_dir, "checkpoints", "best.ckpt")  # Use the best checkpoint if it exists.
        if not os.path.isfile(args.load):
            print("Using the latest checkpoint instead of the best.")
            args.load = os.path.join(save_dir, "checkpoints", "latest.ckpt")
    ...
    if args.eval:
        ...
        img_names = [os.path.basename(x[0]).split(".")[0] for x in validation.data_set.imgs]
        ...
        for i, (cover, _) in tqdm(enumerate(validation)):
            cover = cover.cuda()  # 如果需要在 GPU 上进行运算，保留这行
            # 记录开始时间
            ...
            end_time = time.time()
            # 计算并存储执行时间（秒）
            times.append((end_time - start_time) * 1000)  # 将时间转换为毫秒
