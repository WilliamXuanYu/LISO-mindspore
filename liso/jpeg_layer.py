...
class JPEG_Function(mindspore.ops.PrimitiveWithInfer):
    @staticmethod
    def forward(ctx, input):
        ...
        for i in range(batch_size):
            ...
            res.append(mindspore.tensor(jpeg_image).permute(2, 0, 1).unsqueeze(0).to(input.device))
        return 	mindspore.ops.cat(res, axis=0)
