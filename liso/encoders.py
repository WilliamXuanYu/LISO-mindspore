...
from mindspore.dataset.vision import RandomCrop, CenterCrop
...
from SiaStegNet.src.models import KeNet, XuNet
...
class ContextEncoder(nn.Cell):
    ...
    def construct(self, image, data):
        x = self.features(image)
        x = ops.cat([x] + [data], axis=1)
        x = self.layers(x)
...
class BasicEncoder(nn.Cell):
    ...
    def construct(self, image, data, init_noise=False, verbose=False):
        cnet = self.cnet(image, data)
        net, inp = ops.split(cnet, [self.hdim, self.cdim], axis=1)
        net = ops.tanh(net)
        inp = ops.relu(inp)

        perturb = image.clone()
        if init_noise:
            perturb = perturb + (ops.randn(image.size()) * 0.05).to(perturb)
        ...
        for itr in range(self.iters):
            ...
            perturb = ops.clamp(perturb, -1, 1)
            if self.constraint is not None:
                perturb = ops.clamp(perturb, image - self.constraint, image + self.constraint)
